...
async def add_security_headers(request: Request, call_next):
    ...
    # Relax Content-Security-Policy for Swagger / Redoc documentation endpoints
    path = request.url.path
    if path in ["/docs", "/redoc", "/openapi.json"]:
        response.headers["Content-Security-Policy"] = (
            "default-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://fastapi.tiangolo.com data:; "
        ...
        response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: http://localhost:8001 /static/ https://fastapi.tiangolo.com;"
    return response
